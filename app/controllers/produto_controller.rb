class ProdutoController < ApplicationController
	...
	def save_angular
		...
		if Produto.where(:id => params[:id]).present? 
			...
			@produto.industrializado = data_hash[:industrializado]
			...
			if data_hash[:personalizado] == true
				@produto = Produto.new(descricao: data_hash[:descricao], codigo: data_hash[:codigo], unidade: data_hash[:unidade], preco: data_hash[:preco], industrializado: data_hash[:industrializado],
	...
	def produto_actions
		...
		if current_user.user_type == 2
			...
			if current_user.nivelacesso.acessos.include?(Acesso.find_by_acao('produto#destroy')) || current_user.nivelacesso.acessos.include?(Acesso.find_by_acao('produto#statusset'))
